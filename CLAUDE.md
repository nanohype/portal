...
- **Auth**: GitHub OAuth → JWT, RBAC with roles owner > admin > operator > viewer. A JWT never rides in a URL: the login handoff sets a short-lived HttpOnly `auth_token` cookie scoped to `POST /api/v1/auth/handoff`, which the SPA callback exchanges once for the JWT (the response expires the cookie — delete-on-read), so the token is never exposed in a redirect chain, a `Referer`, or a server log. Once exchanged the SPA holds it in `localStorage` (`web/src/stores/auth.ts`) and sends it as a `Bearer` header, which is script-readable by design — the handoff protects the token in transit, not at rest. WebSockets authenticate via the `["bearer", <jwt>]` subprotocol (`Sec-WebSocket-Protocol`), which is what keeps it out of the WebSocket URL too
...
- **Response helpers**: `internal/handler/respond/respond.go` — `respond.JSON()`, `respond.Error()`, `respond.NoContent()`, `respond.List()` (wraps a slice so an empty result serializes as `[]` rather than `null`), `respond.FromError()` (maps a service error once — see Key Patterns), and `respond.ErrorWithRequest()`, which is `Error()` plus the chi request id in the envelope. `ErrorWithRequest` is the one that changes the wire shape, so reach for it on 5xx where a caller needs to quote an id back at you; the plain `Error()` is for a 4xx the caller can fix without help
