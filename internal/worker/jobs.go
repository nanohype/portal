...
func (w *RunJobWorker) Work(ctx context.Context, job *river.Job[RunJobArgs]) error {
	...
	return w.settleFinishedRun(ctx, args, result, finalStatus, runNotices, logger)
}
...
// notices are what the finish write already put on the run row. They are carried
// here because the withdrawal rewrites that row: a run whose plan diff was lost
// and whose auto-apply was then not enqueued has two things to say, and a
// rewrite that carries only its own drops the first.
func (w *RunJobWorker) settleFinishedRun(ctx context.Context, args RunJobArgs, result *executor.ExecuteResult, finalStatus string, notices []string, logger *slog.Logger) error {
	...
	if finalStatus == "queued" {
		if enqErr := w.enqueueAutoApply(ctx, args); enqErr != nil {
			...
			finalStatus = w.withdrawAutoApplyPromise(ctx, args, result, enqErr, notices, logger)
		} else {
