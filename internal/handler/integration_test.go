// Integration harness for the handler layer, run against a real Postgres.
//
// Most handler tests are pure — they exercise the decision functions directly.
// A few cannot be: an authorization gate that reads the database decides on
// what a query answers, so proving the gate holds means running the handler,
// the service and the SQL together and then looking at the table afterwards.
//
// Like the repository and service harnesses, this creates a scratch database
// (portal_handler_test) off TEST_DATABASE_URL, migrates it, and drops it
...
func TestMain(m *testing.M) {
	...
	const dbName = "portal_handler_test"
...
func execSQL(t *testing.T, ctx context.Context, sql string, args ...any) {
	t.Helper()
...
// seedOrg creates an organization and an owner user in it. The user is only
// ever a foreign key here — the role a request carries comes from its context,
// not from this row.
func seedOrg(t *testing.T, ctx context.Context, slug string) (orgID, userID string) {
	t.Helper()
	orgID, userID = newID(), newID()
	execSQL(t, ctx, `INSERT INTO organizations (id,name,slug) VALUES ($1,$2,$3)`, orgID, slug, slug+"-"+orgID)
	execSQL(t, ctx, `INSERT INTO users (id,org_id,email,name,role) VALUES ($1,$2,$3,'U','owner')`, userID, orgID, userID+"@t.local")
	return orgID, userID
}
