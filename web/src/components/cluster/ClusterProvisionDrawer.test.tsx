...
describe("ClusterProvisionDrawer private-by-default posture", () => {
  it("defaults the public endpoint toggle off and submits a private order", async () => {
    ...
    renderWithClient(
      <ClusterProvisionDrawer open onClose={vi.fn()} accounts={[ACCOUNT]} />,
    );

    expect(
      screen.getByRole("checkbox", { name: /public api endpoint/i }),
    ).not.toBeChecked();
  ...
  it("requires a CIDR allowlist once public access is toggled on", async () => {
    ...
    expect(
      screen.getByText(/required to enable the public endpoint/i),
    ).toBeInTheDocument();
    ...
    expect(submit).toBeDisabled();
    expect(
      screen.getByText(/each entry must be a cidr/i),
    ).toBeInTheDocument();
  ...
  it("sends the allowlist with a public order", async () => {
    ...
    expect(body.endpoint_public_access_cidrs).toEqual([
      "203.0.113.0/24",
      "198.51.100.7/32",
    ]);
  });
