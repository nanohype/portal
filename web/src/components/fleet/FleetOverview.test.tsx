...
describe("FleetOverview", () => {
  it("shows a green verdict for an all-healthy fleet", async () => {
    ...
    expect(
      await screen.findByText(/all 2 clusters healthy/i),
    ).toBeInTheDocument();
  });
  ...
  it("flags how many clusters need attention", async () => {
    mock([
      cl({}),
      cl({ connection_status: "failed" }),
      cl({ control_plane_status: "FAILED" }),
    ]);
    renderWithClient(<FleetOverview />);
    expect(
      await screen.findByText(/2 of 3 clusters need attention/i),
    ).toBeInTheDocument();
  });
