...
import type { Approval, RunStatus } from '@/api/models';
...
export function ApprovalPanel({ workspaceId, runId, runStatus }: Props) {
  ...
  const { data: approvals, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await api.GET('/workspaces/{workspaceId}/runs/{runId}/approvals', {
      ...
      {isLoading ? (
        ...
        <div className="space-y-2 mb-3">
          {(approvals as Approval[]).map((a) => (
            <div key={a.id} className="flex items-start gap-2 text-sm p-2 rounded bg-accent/30">
              {a.avatar_url ? (
                <img src={a.avatar_url} alt="" className="w-5 h-5 rounded-full mt-0.5" />
              ) : (
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={
                      a.status === 'approved' ? 'text-success text-xs' : 'text-destructive text-xs'
                    }
                  >
      ...
      {needsApproval && (
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {approveMutation.isPending ? <Spinner /> : <Check className="w-3.5 h-3.5" />}
              Approve & Apply
            </Button>
            <Button
              size="sm"
