...
function parseAction(header: string): ResourceBlock['action'] {
  ...
  if (header.includes('will be updated') || header.includes('will be changed')) return 'update';
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="flex items-center gap-4 text-sm font-mono">
            {groups.create.length > 0 && (
              <span className="text-green-400">+{groups.create.length} to add</span>
            )}
            {groups.update.length > 0 && (
              <span className="text-yellow-400">~{groups.update.length} to change</span>
            )}
            {groups.replace.length > 0 && (
              <span className="text-orange-400">±{groups.replace.length} to replace</span>
            )}
            {groups.destroy.length > 0 && (
              <span className="text-red-400">-{groups.destroy.length} to destroy</span>
            )}
