  ...
  return (
    <div className="rounded-lg border border-border bg-accent/30 p-4 space-y-2">
      ...
      <p className="text-xs text-muted-foreground">
        ...
        <code className="text-xs">push</code> events only.
      </p>
...
export function WorkspaceSettings({ workspace }: Props) {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: FormValues) => {
      const { data: result, error } = await api.PUT('/workspaces/{workspaceId}', {
    ...
    <div className="max-w-2xl space-y-8">
      ...
      <form onSubmit={handleSubmit((data) => updateMutation.mutate(data))} className="space-y-4">
        ...
        {!isUpload && (
          <>
            ...
            {workspace.vcs_trigger_enabled && <WebhookURLField />}
          </>
        )}
      ...
      <div className="border border-destructive/30 rounded-lg p-6 space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          Deleting a workspace is permanent and cannot be undone. All runs, state versions, and
