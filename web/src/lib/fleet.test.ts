...
describe('clusterState', () => {
  ...
  it('is attention on a failed connection or degraded ArgoCD/control-plane', () => {
    expect(clusterState(cluster({ connection_status: 'failed' }))).toBe('attention');
    expect(clusterState(cluster({ argocd_health_status: 'Degraded' }))).toBe('attention');
    expect(clusterState(cluster({ argocd_health_status: 'Missing' }))).toBe('attention');
    expect(clusterState(cluster({ control_plane_status: 'FAILED' }))).toBe('attention');
  });

  it('is transitional while connecting / progressing / updating', () => {
    expect(clusterState(cluster({ connection_status: 'connecting' }))).toBe('transitional');
    expect(clusterState(cluster({ argocd_health_status: 'Progressing' }))).toBe('transitional');
    expect(clusterState(cluster({ control_plane_status: 'UPDATING' }))).toBe('transitional');
  });

  it('ranks attention over transitional when both apply', () => {
    expect(
      clusterState(cluster({ connection_status: 'failed', control_plane_status: 'UPDATING' })),
    ).toBe('attention');
  });
});
