...
export function summarizeFleet(clusters: Cluster[], operations: ClusterOperation[]): FleetSummary {
  ...
  return {
    ...
    inFlightVends: operations.filter((o) => o.operation === 'provision' && inFlight(o)).length,
    inFlightDeprovisions: operations.filter((o) => o.operation === 'deprovision' && inFlight(o))
      .length,
    versionSpread: tally(clusters.map((c) => c.k8s_version || 'unknown')).map(({ key, count }) => ({
      version: key,
      count,
    })),
    byEnvironment: tally(clusters.map((c) => c.environment)).map(({ key, count }) => ({
      environment: key,
      count,
    })),
    green: attention === 0,
